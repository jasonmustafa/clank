...
export interface DiscordMessageLike {
  ...
  channel: { isThread(): boolean; parentId?: string | null }; attachments: ReadonlyMap<string, { name: string; url: string; size: number; contentType?: string | null }>; inGuild(): boolean;
}
export function normalizeDiscordMessage(message: DiscordMessageLike): DiscordRequest {
  ...
  return { id: message.id, userId: message.author.id, channelId: threadId === null ? message.channelId : (message.channel.parentId ?? message.channelId), threadId,
    guildId: inGuild ? message.guildId : null, location: inGuild ? "guild" : "dm", content: message.content, attachments: [...message.attachments.values()].map(({ name, url, size, contentType }) => ({ name, url, size, contentType: contentType ?? null })), authorIsBot: message.author.bot, webhookId: message.webhookId };
}
...
class DiscordJsTransport implements DiscordTransport {
  ...
  async send(channelId: string, content: string, options?: { kind?: "preview" | "status" | "final"; files?: readonly string[] }): Promise<void> {
    ...
    const chunks = chunkDiscordMessage(content); for (const [index, chunk] of chunks.entries()) await channel.send(index === chunks.length - 1 && (options?.files?.length ?? 0) > 0 ? { content: chunk, files: [...(options?.files ?? [])] } : chunk);
  }
  ...
  async #textChannel(channelId: string): Promise<{ send(content: string | { content: string; files: string[] }): Promise<Message>; sendTyping(): Promise<unknown> }> {
    ...
    return channel as unknown as { send(content: string | { content: string; files: string[] }): Promise<Message>; sendTyping(): Promise<unknown> };
  }
}
...
export async function startV2DiscordGateway(discordToken: string, policy: SuperuserRoutingPolicy, pi: SuperuserPiFactory, store?: TaskStore, attachments?: TaskAttachmentBridge): Promise<V2DiscordGateway> {
  const client = createV2DiscordClient(); const transport = new DiscordJsTransport(client); const router = new SuperuserRequestRouter(policy, transport, pi, store, attachments);
