...
class DiscordJsTransport implements DiscordTransport {
  ...
  async send(channelId: string, content: string, options?: { kind?: "preview" | "status" | "final"; files?: readonly string[] }): Promise<string | undefined> {
    ...
    const chunks = chunkDiscordMessage(content); let last: Message | undefined; for (const [index, chunk] of chunks.entries()) last = await channel.send(index === chunks.length - 1 && (options?.files?.length ?? 0) > 0 ? { content: chunk, files: [...(options?.files ?? [])] } : chunk); return last?.id;
  }
