...
export class SuperuserRequestRouter {
  ...
  async #run(task: Task, prompt: string, request: DiscordRequest, supplied?: { prompt: string; images: readonly ImageContent[] }): Promise<RouteResult> {
    const attachment = supplied ?? await this.#ingest(task, request); const session = await this.#session(task); const generation = task.generation; task.record.lifecycleState = "active"; task.record.updatedAt = new Date().toISOString(); await this.#persist(); let preview = ""; let previewUpdate = this.#discord.updatePreview(task.record.threadId, "⏳ Working…");
    try { const response = await session.prompt(prompt + attachment.prompt, attachment.images, (event) => { if (event.kind === "text") { preview += event.text; previewUpdate = previewUpdate.then(() => this.#discord.updatePreview(task.record.threadId, `⏳ Working…\n\n${preview.slice(-1_500)}`)).catch(() => undefined); } }); await previewUpdate; if (task.generation === generation) { const files = this.#attachments?.outputFor(task.record.id).take() ?? []; try { await this.#discord.send(task.record.threadId, `✅ Finished\n\n${response || "Task completed without text output."}`, { kind: "final", files }); } finally { await this.#attachments?.cleanupFiles(files); } } else await this.#discord.updatePreview(task.record.threadId, "⏹️ Stopped."); return { kind: "completed", taskId: task.record.id }; }
    catch (error) { await previewUpdate; await this.#discord.updatePreview(task.record.threadId, task.generation === generation ? "❌ Failed." : "⏹️ Stopped."); throw error; }
