...
export type PiProgress = { kind: "text"; text: string } | { kind: "tool"; id: string; name: string; status: "started" | "completed" | "failed"; summary?: string };
...
export function formatProgressPreview(text: string, progress: readonly PiProgress[]): string {
  const toolLines = progress.filter((event): event is Extract<PiProgress, { kind: "tool" }> => event.kind === "tool").map((tool) => {
    ...
    return `${icon} \`${tool.name}\`${tool.summary === undefined ? "" : ` — ${tool.summary}`}`;
  });
