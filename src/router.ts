...
export type PiProgress = { kind: "text"; text: string } | { kind: "tool"; id: string; name: string; status: "started" | "completed" | "failed" };
export interface DiscordTransport { createThread(requestId: string, name: string): Promise<string>; send(channelId: string, content: string, options?: { kind?: "preview" | "status" | "final"; files?: readonly string[]; preservePreview?: boolean; approval?: { id: string; taskId: string; command: string } }): Promise<string | undefined>; updatePreview(channelId: string, content: string, page?: number): Promise<void>; }
...
export class SuperuserRequestRouter {
  ...
  async #run(task: Task, prompt: string, request: DiscordRequest, supplied?: { prompt: string; images: readonly ImageContent[] }): Promise<RouteResult> {
    const attachment = supplied ?? await this.#ingest(task, request); const session = await this.#session(task); const generation = task.generation; task.record.lifecycleState = "active"; task.record.updatedAt = new Date().toISOString(); await this.#persist(); let preview = ""; const tools: PiProgress[] = []; const previewUpdates = new Map<number, ReturnType<typeof coalescedPreviewUpdater>>(); const updater = (page: number) => { let result = previewUpdates.get(page); if (result === undefined) { result = coalescedPreviewUpdater((content) => this.#discord.updatePreview(task.record.threadId, content, page)); previewUpdates.set(page, result); } return result; }; const updatePage = (page: number) => { updater(page).push(formatProgressPreview(page === Math.floor(Math.max(0, tools.length - 1) / 8) ? preview : "", tools.slice(page * 8, page * 8 + 8))); }; updater(0).push("⏳ Working…");
    try { const response = await session.prompt(prompt + attachment.prompt, attachment.images, (event) => { if (event.kind === "text") { preview += event.text; updatePage(Math.floor(Math.max(0, tools.length - 1) / 8)); } else { const existing = tools.findIndex((tool) => tool.kind === "tool" && tool.id === event.id); if (existing < 0) { tools.push(event); updatePage(Math.floor((tools.length - 1) / 8)); } else { tools[existing] = { ...tools[existing], ...event }; updatePage(Math.floor(existing / 8)); } } }); await Promise.all([...previewUpdates.values()].map((updates) => updates.finish())); if (task.generation === generation) { const files = this.#attachments?.outputFor(task.record.id).take() ?? []; try { await this.#discord.send(task.record.threadId, `${response || "Task completed without text output."}\n\n✅ Finished`, { kind: "final", files, preservePreview: tools.length > 0 }); } finally { await this.#attachments?.cleanupFiles(files); } } else await this.#discord.updatePreview(task.record.threadId, "⏹️ Stopped.", Math.floor(Math.max(0, tools.length - 1) / 8)); return { kind: "completed", taskId: task.record.id }; }
    catch (error) { await Promise.all([...previewUpdates.values()].map((updates) => updates.finish())); await this.#discord.updatePreview(task.record.threadId, task.generation === generation ? "❌ Failed." : "⏹️ Stopped.", Math.floor(Math.max(0, tools.length - 1) / 8)); throw error; }
...
function coalescedPreviewUpdater(update: (content: string) => Promise<void>): { push(content: string): void; finish(): Promise<void> } {
  let active: Promise<void> | undefined; let latest = ""; let pending = false; let closed = false;
  const pump = () => { if (active !== undefined || closed || !pending) return; pending = false; const content = latest; active = update(content).catch(() => undefined).finally(() => { active = undefined; pump(); }); };
  return { push(content) { if (closed) return; latest = content; pending = true; pump(); }, async finish() { const finalContent = pending ? latest : undefined; closed = true; pending = false; await active; if (finalContent !== undefined) await update(finalContent).catch(() => undefined); } };
}
...
export function formatProgressPreview(text: string, progress: readonly PiProgress[]): string {
  const toolLines = progress.filter((event): event is Extract<PiProgress, { kind: "tool" }> => event.kind === "tool").map((tool) => {
    const icon = tool.status === "started" ? "🔧" : tool.status === "completed" ? "✅" : "❌";
    return `${icon} \`${tool.name}\``;
  });
  const toolSection = toolLines.join("\n"); const status = "⏳ Working…"; const separators = toolSection === "" || text === "" ? 2 : 4; const availableText = Math.max(0, 2_000 - toolSection.length - status.length - separators);
  const sections = [toolSection, text.slice(-Math.min(1_500, availableText)), status].filter((section) => section !== "");
