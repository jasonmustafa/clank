...
export class SuperuserRequestRouter {
  ...
  async #run(task: Task, prompt: string, request: DiscordRequest, supplied?: { prompt: string; images: readonly ImageContent[] }): Promise<RouteResult> {
    const attachment = supplied ?? await this.#ingest(task, request); const session = await this.#session(task); const generation = task.generation; task.record.lifecycleState = "active"; task.record.updatedAt = new Date().toISOString(); await this.#persist(); let preview = ""; const previewUpdates = coalescedPreviewUpdater((content) => this.#discord.updatePreview(task.record.threadId, content)); previewUpdates.push("⏳ Working…");
    try { const response = await session.prompt(prompt + attachment.prompt, attachment.images, (event) => { if (event.kind === "text") { preview += event.text; previewUpdates.push(`⏳ Working…\n\n${preview.slice(-1_500)}`); } }); await previewUpdates.finish(); if (task.generation === generation) { const files = this.#attachments?.outputFor(task.record.id).take() ?? []; try { await this.#discord.send(task.record.threadId, `✅ Finished\n\n${response || "Task completed without text output."}`, { kind: "final", files }); } finally { await this.#attachments?.cleanupFiles(files); } } else await this.#discord.updatePreview(task.record.threadId, "⏹️ Stopped."); return { kind: "completed", taskId: task.record.id }; }
    catch (error) { await previewUpdates.finish(); await this.#discord.updatePreview(task.record.threadId, task.generation === generation ? "❌ Failed." : "⏹️ Stopped."); throw error; }
...
function coalescedPreviewUpdater(update: (content: string) => Promise<void>): { push(content: string): void; finish(): Promise<void> } {
  ...
  return { push(content) { if (closed) return; latest = content; pending = true; pump(); }, async finish() { closed = true; pending = false; await active; } };
}
